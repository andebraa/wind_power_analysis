...
def preprocess(sentences):
    ...
    for i in range(0,len(sentences)):
        ...
        sentences[i] = [word for word in sentences[i].split() if word not in stopword]
        sentences[i] = ' '.join(sentences[i])
    return sentences


def roberta_threelabel(lr = 1e-5, batch_size = 16, epochs = 10, plot = False, predict = False):
    infile = 'annotation_1845_5800unskewed_wli'
...
if __name__ == '__main__':
    roberta_threelabel(lr = 1e-5, batch_size = 32, epochs = 5, plot=True, predict = False)
