...
pd.options.mode.chained_assignment = None #default='warn' #NOTE this is an important ignore
...
def remove_searchwords(tweet_df):
    '''
    for when making wordcloud the searchwords occur in most all of tweets
    thus dominating the wordcloud.
    '''
    for i, tweet in tweet_df.iteritems():
        tweet = tweet.lower()
        #remove searchwords
        havvind_regex = '(?:^|(?<= )|\W)(havvind|Havvind|vindkraft|Vindkraft|vindmølle|Vindmølle|vindmøller|Vindmøller|vindmøllene|Vindmøllene|vindturbiner|Vindturbiner|vindenergi|Vindenergi)(?:$|(?= )|\W)'
        print(tweet)
        search_res = re.search(havvind_regex, tweet, re.I| re.M)
        while search_res:
            print('-'*20)
            print(search_res)
...
def roberta_sentiment(lr = 1e-5, batch_size = 16, epochs = 10, 
                      plot = False, predict = False, labelcomb = 'posneutral'):
    '''
    Script for training, testing and applying Bert for sentiment classification.
    This script requires two labels to be combined 
    '''
    #infile = f'annotation_5900_posneutral_0neg_1pos_300wli_cleaned'
    #infile = f'annotaion_5900_01label_comb_posneutral_0neg_1pos_300iwl_cleaned_nouser'
    ...
    if wordcloud:
        truemask = df['label']==1
        falsemask = df['label']==0

        truetweets = df['text'][truemask].copy()
        falsetweets = df['text'][falsemask].copy()
        alltweet = df['text'].copy()
        
        truetweets = remove_searchwords(truetweets)
        falsetweets = remove_searchwords(falsetweets)
        tweet_all = remove_searchwords(alltweet)

        fig, ax = plt.subplots(3,1)#, figsize = (30,30))

        tweet_pos = ' '.join(elem for elem in truetweets)
        tweet_neg = ' '.join(elem for elem in falsetweets)
        tweet_all = ' '.join(elem for elem in alltweet)
        ...
        ax[1].set_title('non-negative tweets')#, fontsize = 10)
        ax[1].axis('off')

        ax[2].imshow(wordcloud_neg, interpolation = 'bilinear')
        ax[2].set_title('negative tweets')#, fontsize = 10)
        ax[2].axis('off')

        plt.savefig('../fig/wordcloud_roberta_solar_posneutral_posneutral_cleaned.png', dpi = 200)
        plt.clf()
    ...
    for sent in sentences:
        ...
        encoded_dict = tokenizer.encode_plus(
                            ...
                            max_length = max_length,  #max_len? Pad & truncate all sentences.
                            truncation = True,
    ...
    model = BertForSequenceClassification.from_pretrained(
        ...
        "NbAiLab/nb-bert-large", #nationalbiblioteket 
    ...
    #this overwrites validation acc. is actually test accuracy
    total_eval_accuracy=0
    # Predict
    ...
    runs_folder = f'runs/epochs{epochs}_batch_size{batch_size}_lr{lr}_{labelcomb}_f1{f1:.2f}_acc{avg_val_accuracy:.2f}'
    folder = False
    folder_number = 1
    ...
    df_stats.to_csv(runs_folder+f'/training_stats_epochs{epochs}_batch_size{batch_size}_lr{lr}_{labelcomb}.csv')
    header = f'epochs{epochs}_batch_size{batch_size}_lr{lr}_{labelcomb}_f1{f1}_acc{avg_val_accuracy}'
    ...
    if plot:
        fig, ax = plt.subplots()
        ax.plot(epochs_arr, validation_accuracy, label = 'val acc')
        ax.plot(epochs_arr, training_loss, label = 'training loss')
        ax.plot(epochs_arr, validation_loss, label = 'val loss')
        ax.set_xticks(epochs_arr)
        ax.set_xlabel('epochs')
        ax.set_ylabel('loss')
        ax.legend()
        fig.suptitle(f'batch length {batch_size}, final f1 {f1:.2f}, test accuracy {avg_val_accuracy:.2f}')
        fig.savefig(runs_folder +f'/lossplot_{batch_size}_epochs{epochs}'+'.png', dpi = 200)
        plt.clf()
        #plt.show()
        conf_matrix = True
        ...
        if conf_matrix:
            ...
            plt.savefig(runs_folder  +f'/confusion_matrix_{labelcomb}_epochs{epochs}_f1{f1:.2f}.png')
            plt.clf()
    ...
    predict_dataset = '/home/users/andebraa/wind_power_analysis/data/fourth_rendition_data/'
    #second_rendition_geolocated_anonymous.csv' 
    if predict:
        from itertools import chain
        #filename = 'fourth_rendition_geolocated_id_cleaned.csv' 
        filename = 'fourth_rendition_geolocated_id_cleaned_walphanumerical.csv'
        ...
        # Convert the lists into tensors.
            ...
            out_filename = filename.split('.')[0]+'nb_predict'+'.csv'
            #out_filename = f'fourth_rendition_geolocated_id_{labelcomb}_predict_nouser.csv'
...
if __name__ == '__main__':
    ...
    #gridsearch()
    roberta_sentiment(lr = 4.002e-06, batch_size = 32, epochs = 20, plot=True, predict = False)
