...
data = pd.read_csv('final_dataset.csv', parse_dates = True)# , usecols = [''])
...
plt.savefig('final_dataset_userhist.jpg', bbox_inches = 'tight', pad_inches = 0.1) #0.1 is default when bbox is tight

outfile_ID = open('final_dataset_translator.json', 'w')
json.dump(ID, outfile_ID) #writing the translation dictionary to file

data.to_csv('final_dataset_anonymous.csv', index=False)
