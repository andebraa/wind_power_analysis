    ...
    for i, elem in data.iterrows():
        ...
        if elem['username'] not in user_dict:
            ...
            user_dict[elem['username']].append([elem['text'],
                                                ...
                                                elem['logits1']])
    if save:
        with open('third_rendition_geolocated_anonymous_usernamedict.json' ,'w') as fp:
...
ax.plot(week_nums, flat[:len(week_nums)], label='full data')
ax2[0].plot(week_nums, flat_trim_99[:len(week_nums)], label='99%', alpha = 0.7)
ax2[0].plot(week_nums, flat_trim_95[:len(week_nums)], label='95%', alpha = 0.7)
ax2[0].legend()
ax2[1].plot(week_nums, flat_trim_90[:len(week_nums)], label='90%', alpha = 0.7)
ax2[1].legend()
ax2[1].plot(week_nums, flat_trim_85[:len(week_nums)], label='85%', alpha = 0.7)
ax2[1].legend()
ax2[0].set_ylabel('number of tweets')
ax2[1].set_ylabel('number of tweets')
#ax2.set_suptitle('tweets per week')
plt.legend()
plt.ylabel('number of tweets')
plt.suptitle('tweets per week')

fig2.savefig('tweet_per_week_third_rendition_geolocated_trimmed_foursquare2.png', format='png', bbox_inches = 'tight', pad_inches = 0.1, dpi = 300) #0.1 is default when bbox is tigh
